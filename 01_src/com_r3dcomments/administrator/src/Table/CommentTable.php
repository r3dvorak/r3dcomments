...
namespace Joomla\Component\R3dcomments\Administrator\Table;
...
class CommentTable extends Table
{
    ...
    {
        parent::__construct('#__r3dcomments', 'id', $db);
    }
    ...
    public function bind($data, $ignore = '')
    {
        ...
        if(!Factory::getUser()->authorise('core.admin', 'com_r3dcomments.comment.'.$data['id']))
        {
            ...
            foreach($actions as $action)
            {
                ...
                if (!$actionName || !isset($defaultActions[$actionName])) {
                    // Fallback: Regel existiert nicht → komplett verweigern
                    $jaccessRules[$actionName] = [];
                    continue;
                }

                $jaccessRules[$actionName] = $defaultActions[$actionName];
            }
    ...
    {
        $k = $this->_tbl_key;
        return 'com_r3dcomments.comment.' . (int) $this->$k;
    }
	 ...
	 * @return int
	 */
    protected function _getAssetParentId(?Table $table = null, mixed $id = null) : int
    {
        ...
        $assetParent->loadByName('com_r3dcomments');
